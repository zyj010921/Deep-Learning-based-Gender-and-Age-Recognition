...
model.load_state_dict(torch.load("./models/best_age{:.3f}_gen{:.3f}.pth".format(max_age_acc, max_gen_acc)))

# 冻结性别分类层和特征提取层
# for param in model.gender_fc_layers.parameters():
...
for epoch in range(num_epochs):
    ...
    for sample_batched in tqdm(dataloader):
        ...
        # loss = mse_loss(m_age_out, age_batch) + cross_loss(m_gender_out, gender_batch)
    ...
    with torch.no_grad():
        ...
        for sample_batched in tqdm(testloader):
            images_batch, age_batch, gender_batch = sample_batched['image'], sample_batched['age'], sample_batched[
                'gender']
            if train_on_gpu:
                images_batch, age_batch, gender_batch = images_batch.cuda(), age_batch.cuda(), gender_batch.cuda()
        ...
        if gen_acc > max_gen_acc:
            ...
            # torch.save(model.state_dict(), "./models/best_age{:.3f}_gen{:.3f}.pth".format(age_acc,max_gen_acc))
